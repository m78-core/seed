...
export type Share<D, V> = CreateKitConfig<D, V> & {
...
export type Listener<S extends object = AnyObject> = (changes: Partial<S>) => void;
...
export interface Auth<S, V> {
  /** 更新state */
  setState: SetState<S & { [key: string]: any }>;
  /** 以新state覆盖当前state */
  coverSetState: CoverSetState<S & { [key: string]: any }>;
